...
We could use PhyloBuild.sh to generate a tree and use this to plot the carriage. The typical steps of this script are below, but of course depend on what parameters the user has chosen:
   ...
   5. This generates a concatenated file of the alignments, which is great but we aren't done yet. We need to make the tree with PhyML, however we cannot use a .aln file to generate this. So, we need to convert it to a phylip file format, using trimAl, which also runs a trimming step of the alignment as well. This simply generates a .phy file, which I've designated phylipfor.phy.
